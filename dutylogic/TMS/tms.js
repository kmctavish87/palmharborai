...
async function loadHubData(state, elements) {
  ...
  try {
    ...
    setStatus(
      elements,
      "There was a problem loading TMS data. Check runtime configuration and refresh settings."
    );
  }
}
...
function renderHub(state, elements) {
  ...
  renderItemList(filteredStudies, "study", state, elements.studyList, elements);
  renderItemList(filteredNews, "news", state, elements.newsList, elements);
...
function renderItemList(items, type, state, container, elements) {
  container.innerHTML = "";
  ...
  items.forEach((item) => {
    ...
    const tagMarkup = tags.length
      ? `<div class="tms-card-tags">${tags
          ...
          .join("")}</div>`
      : "";
    const sourceLinkMarkup = url
      ? `<a class="tms-link-button" href="${escapeAttribute(url)}" target="_blank" rel="noreferrer">Open source</a>`
      : "";
...
async function generateDraft(state, elements) {
  ...
  try {
    ...
    console.error(error);
    setStatus(elements, "The draft could not be generated. Check API configuration and try again.");
  } finally {
